...
if !exists("tex_no_math")
 ...
 " Modification by Michael Goerz:
 " deleted some cluster definitions texDocGroup, texPartGroup, texChapterGroup,
 " texSectionGroup, ... 
endif
...
syn match texInputFile		"\\include\(graphics\|list\)\=\(\[.\{-}\]\)\=[[:space:]\n]*{.\{-}}"	contains=texStatement,texInputCurlies
...
" Modification by Michael Goerz:
" Deleted some regions: texPartZone, texChapterZone, texChapterZone...

" Define a few zones: {{{1
if g:tex_fold_enabled && has("folding")
 syn region texDocZone                  matchgroup=texSection start='\\begin\s*{\s*document\s*}' end='\\end\s*{\s*document\s*}' fold contains=@texFoldGroup,@texDocGroup,@Spell, texSectionTitle
 ...
 syn region texDocZone                  matchgroup=texSection start='\\begin\s*{\s*document\s*}' end='\\end\s*{\s*document\s*}' contains=@texFoldGroup,@texDocGroup,@Spell, texSectionTitle
...
" Special highlighting for titles of section/subsection etc. {{{1 
" (Modification by Michael Goerz)"
syntax region texSectionTitle matchgroup=texSection start="\\part\*\=\(\[.*\]\)*{" end="}" contains=@Spell
syntax region texSectionTitle matchgroup=texSection start="\\chapter\*\=\(\[.*\]\)*{" end="}" contains=@Spell
syntax region texSectionTitle matchgroup=texSection start="\\section\*\=\(\[.*\]\)*{" end="}" contains=@Spell
syntax region texSectionTitle matchgroup=texSection start="\\subsection\*\=\(\[.*\]\)*{" end="}" contains=@Spell
syntax region texSectionTitle matchgroup=texSection start="\\subsubsection\*\=\(\[.*\]\)*{" end="}" contains=@Spell
syntax region texSectionTitle matchgroup=texSection start="\\paragraph\*\=\(\[.*\]\)*{" end="}" contains=@Spell
syntax region texSectionTitle matchgroup=texSection start="\\subparagraph\*\=\(\[.*\]\)*{" end="}" contains=@Spell
...
" (Modification by Michael Goerz: added NoSpell)
...
if did_tex_syntax_inits == 1
 let did_tex_syntax_inits= 2
  ...
  HiLink texSectionTitle        Underlined
...
" Activate Spell Checking for Standard Text
syn spell toplevel
